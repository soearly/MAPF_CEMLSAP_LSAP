...
def cem_plan(map_file, my_map, starts, goals, disjoint=False, num_samples=100, num_elite=10, maxiter=20):
    ...
    try:
        ...
        if paths:
            ...
            gif_path = os.path.join("videos", filename_only.replace(".txt", "_cem_animation.gif"))
            animation.save(gif_path, time_step=1.0, writer="pillow")
            print(f"✅ Animation saved to: {gif_path}")
        else:
            print(f"⚠️  No valid paths found for {filename_only}, skipping animation.")
    except Exception as e:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch', action='store_true',
                        help='(Unused) Suppress display (animation is saved regardless)')
